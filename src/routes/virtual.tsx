...
function LiveMatchTicker({ match, animSec }: { match: MatchRow & { lock_time?: string | null }; animSec: number }) {
  ...
  const [fighters, setFighters] = useState<Fighter[]>(() => {
    const arr: Fighter[] = [];
    for (let i = 0; i < 8; i++) arr.push({ x: 8 + seedRand(match.id, i) * 25, y: 10 + seedRand(match.id, i + 100) * 80, side: "h", alive: true, flash: 0, vx: 0.22 + seedRand(match.id, i + 400) * 0.28, vy: -0.18 + seedRand(match.id, i + 500) * 0.36 });
    for (let i = 0; i < 8; i++) arr.push({ x: 67 + seedRand(match.id, i + 200) * 25, y: 10 + seedRand(match.id, i + 300) * 80, side: "a", alive: true, flash: 0, vx: -0.22 - seedRand(match.id, i + 600) * 0.28, vy: -0.18 + seedRand(match.id, i + 700) * 0.36 });
    return arr;
  });
  ...
  useEffect(() => {
    const tick = () => {
      ...
      setFighters((prev) => {
        ...
        const targetAlive = f.side === "h" ? Math.max(0, 8 - Math.min(8, fa)) : Math.max(0, 8 - Math.min(8, fh));
      ...
      if (Math.random() < 0.55) {
        setTracers((prev) => {
          ...
          if (Math.random() < 0.18) setBlasts((old) => [...old, { x: b.x, y: b.y, born: now, size: 18 + Math.random() * 18 }].filter((v) => now - v.born < 900).slice(-5));
  ...
  return (
    <div className="mt-3 rounded-lg border border-destructive/40 bg-destructive/5 overflow-hidden">
      ...
      <div className="relative w-full aspect-[16/9] overflow-hidden bg-[#0b0f0a]">
        ...
        {/* Side labels */}
        <div className="absolute top-1 left-2 text-[9px] font-black tracking-widest text-red-400 drop-shadow">RED · {homeName.toUpperCase()}</div>
        <div className="absolute top-1 right-2 text-[9px] font-black tracking-widest text-sky-400 drop-shadow">{awayName.toUpperCase()} · BLUE</div>
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none" preserveAspectRatio="none" viewBox="0 0 100 100">
          ...
          {tracers.map((t, i) => (
            <line key={i} x1={t.x1} y1={t.y1} x2={t.x2} y2={t.y2}
              stroke={t.side === "h" ? "#ff5252" : "#4dd2ff"} strokeWidth="0.35"
          ...
          <div key={i}
            ...
            {f.alive ? (
              <div className="relative">
                ...
                {f.flash > 0.2 && (
                  <div className="absolute -top-1 -left-1 h-4 w-4 rounded-full bg-amber-300/80 animate-ping" style={{ animationDuration: "0.6s" }} />
                )}
              </div>
            ) : (
        ...
        <div className="space-y-1 min-h-[56px]">
          ...
          {feed.map((line, i) => (
            <div key={i} className="text-[11px] text-foreground/90 animate-fade-in flex items-start gap-1.5">
              <span className="text-destructive">▸</span>{line}
            </div>
          ))}
        </div>
      </div>
