...
export function ReferralCard() {
  ...
  async function applyCode() {
    if (!code.trim()) return;
    setBusy(true);
    const { error, data } = await supabase.rpc("apply_referral_code", { code: code.trim().toUpperCase() });
    setBusy(false);
...
/* ============================ NOTIFICATIONS (in-app only, push skipped) ============================ */
export function PushNotifSettings() {
  ...
  useEffect(() => {
    ...
    supabase.from("notification_prefs").select("*").eq("user_id", user.id).maybeSingle()
      .then(({ data }) => setPrefs(data ?? { user_id: user.id, push_enabled: false, match_starting: true, bet_results: true, withdrawals: true, promotions: true, chat_mentions: true, ticket_replies: true, rewards: true, referrals: true, vip_tier_up: true, daily_streak: true }));
  }, [user?.id]);
  ...
  return (
    <Card className="p-6 glass-card">
      <div className="flex items-center justify-between mb-3">
        ...
        <Badge className="bg-muted text-muted-foreground border-border">Push disabled</Badge>
      </div>
      <p className="text-xs text-muted-foreground mb-4">
        Choose which events show up in your in-app notification bell. Browser push is disabled in this build.
      </p>
      <div className="space-y-2">
        {[
