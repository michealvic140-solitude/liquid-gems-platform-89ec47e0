...
AS $$
DECLARE
  s text := COALESCE(_seed, '') || ':' || _i::text;
  h bigint := 0;
  pos integer;
BEGIN
  FOR pos IN 1..char_length(s) LOOP
    h := mod((h * 31) + ascii(substr(s, pos, 1)), 1000003);
  END LOOP;
...
BEGIN
  ...
  FOR i IN 0..GREATEST(0, event_count - 1) LOOP
    ...
    IF public.virtual_seed_rand(_match_id::text, 960 + i) > 0.48 THEN
...
BEGIN
  ...
  FOR bet IN SELECT DISTINCT b.* FROM public.bets b
    ...
    WHERE bs.match_id = _match_id AND b.status IN ('pending','open','won')
  LOOP
...
BEGIN
  ...
  LOOP
    UPDATE public.matches
       SET status='live', locked_at = now(), updated_at=now()
